/* ======================================================
   MahaVidya — Platform JavaScript
   5-Pillar: Accessibility, Slider, Command Bar, AI Chat, OTP
   ====================================================== */
document.addEventListener('DOMContentLoaded', () => {

    /* ======================================================
       PILLAR 1 — ACCESSIBILITY (GIGW 3.0 / WCAG 2.1)
    ====================================================== */
    ...
    /* ======================================================
       PILLAR 2 — COMMAND BAR (Cmd+K / Ctrl+K)
    ====================================================== */
        ...
        schoolSearch.addEventListener('input', e => {
            ...
            debounceTimer = setTimeout(() => {
                fetch(`http://localhost:8000/api/schools/search?q=${encodeURIComponent(q)}`)
                    ...
                    }).catch(() => { searchDrop.innerHTML = `<div class="p-sm text-muted">Server offline — search unavailable</div>`; });
            }, 300);
        });
    }

    /* ======================================================
       PILLAR 3 — LOGIN MODAL + OTP FLOW
    ====================================================== */
    ...
    if (submitLoginBtn) {
        submitLoginBtn.addEventListener('click', () => {
            ...
            .catch(() => {
                errorBox.querySelector('span.lang-en').textContent = 'Network error — server offline.';
                errorBox.classList.remove('hidden');
                submitLoginBtn.innerHTML = '<span class="lang-en">Sign In Securely</span> <i class="fa-solid fa-arrow-right-to-bracket"></i>';
                submitLoginBtn.disabled = false;
            });
    ...
    /* ======================================================
       PILLAR 4 — VISHWA-MITRA AI CHAT + VOICE
    ====================================================== */
    ...
    const vmResponses = {
        en: {
            default: "I'm here to help! You can ask me about school fees, homework schedules, exam dates, teacher contacts, or student progress. 😊",
            fees: "School fees for the current academic year are ₹0 (ZP schools are free under RTE Act). For uniform allowance or scholarship queries, contact your school's HM.",
            ...
            exam: "Next exam dates: Unit Test 2 — 18 March 2026. Final Exam — 12–20 April 2026. Results within 7 days of exam completion.",
        ...
        mr: {
            default: "मी मदत करण्यासाठी येथे आहे! शाळा शुल्क, गृहपाठ, परीक्षा तारखा, किंवा विद्यार्थी प्रगतीबद्दल विचारा. 😊",
            fees: "ZP शाळांमध्ये RTE कायद्यानुसार शुल्क नाही. शिष्यवृत्ती किंवा गणवेशासाठी मुख्याध्यापकांशी संपर्क करा.",
            homework: "गृहपाठ वेळापत्रक शिक्षक दर शुक्रवारी अपलोड करतात. लॉगिन केल्यानंतर विद्यार्थी पोर्टलवर पहा.",
            attendance: "सध्याची सरासरी उपस्थिती 87.4% आहे. तुमच्या मुलाची उपस्थिती पालक पोर्टलवर पहा.",
            exam: "पुढील परीक्षा: युनिट टेस्ट 2 — 18 मार्च 2026. अंतिम परीक्षा — 12-20 एप्रिल 2026.",
            progress: "मासिक AI प्रगती अहवाल प्रत्येक महिन्याच्या पहिल्या तारखेला उपलब्ध होतो.",
        }
    };
    ...
    function getVMResponse(query) {
        ...
        if (q.includes('fee') || q.includes('शुल्क')) return r.fees;
        if (q.includes('homework') || q.includes('गृहपाठ')) return r.homework;
        if (q.includes('attendance') || q.includes('उपस्थिती')) return r.attendance;
        if (q.includes('exam') || q.includes('परीक्षा')) return r.exam;
        if (q.includes('progress') || q.includes('प्रगती')) return r.progress;
        return r.default;
    }
    ...
    async function loadNoticeBoard() {
        ...
        try {
            ...
            if (resultsList) {
                resultsList.innerHTML = resultsData.map(r =>
                    `<div class="result-row ${r.is_new ? 'result-new' : ''}">
                        ...
                        ${r.is_new ? '<span style="background:#E8F5E9;color:#2E7D32;font-size:.7rem;font-weight:700;padding:3px 10px;border-radius:20px;white-space:nowrap;">NEW ✓</span>' : ''}
                    </div>`
                ).join('');
            }
            ...
            if (ticker) {
                ticker.innerHTML = `<span style="color:rgba(255,255,255,.8);font-size:.82rem;padding:0 2rem;">📢 Unit Test 2 on 18 March 2026 &nbsp;|&nbsp; 👨‍👩‍👧 Parent-Teacher Meeting 22 March &nbsp;|&nbsp; 📋 Unit Test 1 Results Declared &nbsp;|&nbsp; 🎉 Gudi Padwa Holiday 29 March</span>`;
            }
            if (eventsList) {
                eventsList.innerHTML = `<div class="event-card"><div class="event-date-box"><div class="ed-day">18</div><div class="ed-mon">MAR</div></div><div><strong>Unit Test 2 — All Subjects</strong><br><span style="font-size:.78rem;color:#64748B">Class 5–8 | 8:00 AM</span></div></div>`;
            }
        }
    }
