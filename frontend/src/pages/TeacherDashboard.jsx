...
const TeacherDashboard = () => {
    ...
    const handleSendNotice = async () => {
        ...
        try { await api.post('/notices', { title: 'Teacher Notice', message: noticeText, audience: 'All' }); setNoticeText(''); }
                                    ...
                                    <table className="data-table">
                                        ...
                                        <tbody>
                                            {students.length > 0 ? students.map(st => (
                                                <tr key={st.id} className="table-row">
                                                    ...
                                                    <td className="td">
                                                        <div style={{ display: 'flex', gap: '.4rem' }}>
                                                            <button style={{ width: '38px', height: '32px', borderRadius: '6px', border: '1.5px solid var(--success)', background: attendance[st.id] === 'P' ? 'var(--success)' : '#E8F5E9', color: attendance[st.id] === 'P' ? '#fff' : 'var(--success)', fontWeight: 700, cursor: 'pointer' }} onClick={() => markAttendance(st.id, 'P')}>P</button>
                                                            <button style={{ width: '38px', height: '32px', borderRadius: '6px', border: '1.5px solid var(--danger)', background: attendance[st.id] === 'A' ? 'var(--danger)' : '#FFEBEE', color: attendance[st.id] === 'A' ? '#fff' : 'var(--danger)', fontWeight: 700, cursor: 'pointer' }} onClick={() => markAttendance(st.id, 'A')}>A</button>
                                                        </div>
                                                    </td>
                                                ...
                                                {isLoadingStudents ? <><i className="fa-solid fa-spinner fa-spin" /> Loading students...</> : 'No records found'}
                                            </td></tr>)}
                                        </tbody>
                                    </table>
                                </div>
                    ...
                    {activeTab === 'messages' && (
                        <div className="panel slide-in active">
                            <div className="panel-card" style={{ maxWidth: '600px' }}>
                                ...
                                <div className="form-group mb-sm"><label className="form-label">Notice Title</label><input className="form-input" value="Teacher Notice" disabled /></div>
                                <div className="form-group mb-sm"><label className="form-label">Message (English / मराठी)</label><textarea className="form-input" rows="4" placeholder="Type your notice here..." value={noticeText} onChange={e => setNoticeText(e.target.value)} /></div>
                                <button className="btn btn-saffron w-100" onClick={handleSendNotice}>Broadcast to Parents <i className="fa-solid fa-paper-plane" /></button>
                            </div>
                        </div>
                    )}
